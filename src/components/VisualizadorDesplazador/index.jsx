...
import acabados from "../../assets/images/T5/Diseno/Acabados/Dolcker acabados T5.jpg";
		...
		if (isMouseDown) {
			if (posX - initPos > 30) {
				setIndexDesplazador((prevIndex) => {
					setInitPos(posX);
					if (prevIndex === actualDesplazadorImgs.length - 1) return prevIndex;
	...
	if (actualDesplazadorImgs.length === 0) {
		...
		return (
			<StyledVisualizadorDesplazador>
				...
				>
					<Indicador bottom="5%" left="5%" onClick={cleanDesplazador} close />
				</div>
			</StyledVisualizadorDesplazador>
		);
	}
