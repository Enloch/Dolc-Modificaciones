...
const StyledListaContenidos = styled.nav`
	...
	> ul > * {
		// Seleccionar hijos directos de la lista desordenada
		position: relative; // Establecer la posición como relativa para el posicionamiento de pseudo-elementos

		&::before {
			// Usar un pseudo-elemento para mostrar el contador
			content: counter(
				counter,
				decimal-leading-zero
			); // Mostrar el contador con ceros a la izquierda
			...
			right: calc(
				100% + ${GRID.columnGap}
			); // Posicionarlo a la derecha del elemento de la lista
		...
		${mediaQueryTablet(css`
			::before {
				// Ocultar el contador en pantallas más grandes
	...
	> ul > ${StyledSublista} {
		// Seleccionar hijos directos que son StyledSublista
		grid-template-columns: repeat(
			6,
			1fr
		); // Definir un diseño de cuadrícula con 6 columnas iguales

		> ul > ${StyledSublista} {
			// Seleccionar sublistas anidadas
			grid-template-columns: repeat(
				5,
				1fr
			); // Definir un diseño de cuadrícula con 5 columnas iguales

			> ul > ${StyledSublista} {
				// Seleccionar sublistas aún más anidadas
				grid-template-columns: repeat(
					4,
					1fr
				); // Definir un diseño de cuadrícula con 4 columnas iguales
			}
		}
	}
`;
