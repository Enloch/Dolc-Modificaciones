...
const VisualizadorVariaciones = ({ config, srcfondo, altFondo, volumen, visRef }) => {
		...
		return (
			<StyledVisualizadorVariaciones ref={visRef} selectedColor={selectedColor}>
				...
				<div className="indicadores" onClick={handleClick}>
					<IsOpenContext.Provider value={isOpen}>{indicadores}</IsOpenContext.Provider>
					<Indicador
						onClick={() => {
							if (isOpen) {
	...
	return (
		<StyledVisualizadorVariaciones selectedColor={selectedColor}>
			...
			{/* Renderizar el componente Menu */}
			<Menu selectores={selectores} seleccionado={seleccionado} isOpen={isOpen} idMenu={idMenu} changeSeleccionado={changeSeleccionado} />
		</StyledVisualizadorVariaciones>
	);
};
