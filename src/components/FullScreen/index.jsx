...
const FullScreen = ({
      ...
      if (imgSrc[indiceSrc].toLowerCase().includes("honed"))
        setFullScreenTitulo((prevTitulo) => {
          ...
          if (ultimaPalabra === "flamed" || ultimaPalabra === "mate")
            return `${prevTitulo.split(" ").slice(0, -1).join(" ")} Honed`;
      ...
      if (imgSrc[indiceSrc].toLowerCase().includes("flamed"))
        setFullScreenTitulo((prevTitulo) => {
          ...
          if (ultimaPalabra === "honed" || ultimaPalabra === "mate")
            return `${prevTitulo.split(" ").slice(0, -1).join(" ")} Flamed`;
      ...
      if (imgSrc[indiceSrc].toLowerCase().includes("mate"))
        setFullScreenTitulo((prevTitulo) => {
          ...
          if (ultimaPalabra === "flamed" || ultimaPalabra === "honed")
            return `${prevTitulo.split(" ").slice(0, -1).join(" ")} Mate`;
  ...
  if (Array.isArray(imgSrc)) {
    ...
    return (
      <StyledFullScreen display={isShow ? "flex" : "none"} ref={fullScreenRef}>
        <div
          className='slider'
          id={imgsContainerId}
        ...
        <div className='controles'>
          <img src={izq} alt='Desplazar a la izquierda' onClick={desplIzq} />
          <span>{`${indiceSrc + 1} / ${imgSrc.length}`}</span>
          <img src={der} alt='Desplazar a la derecha' onClick={desplDer} />
        </div>
        <img
          src={closeIcon}
          alt='Cerrar menú'
